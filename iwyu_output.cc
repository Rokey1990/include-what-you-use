...
void ProcessForwardDeclare(OneUse* use,
  ...
  const NamedDecl* record_decl = DynCastFrom(use->decl());
  ...
  for (const NamedDecl* redecl : redecls) {
    ...
    if (const TagDecl* record_redecl = DynCastFrom(redecl)) {
      isCompleteDefinition = record_redecl->isCompleteDefinition();
    } else if (const clang::ObjCInterfaceDecl* objc_redecl = DynCastFrom(redecl)) {
      isCompleteDefinition = objc_redecl->isThisDeclarationADefinition();
    } else if (const clang::ObjCProtocolDecl* objc_redecl = DynCastFrom(redecl)) {
      isCompleteDefinition = objc_redecl->isThisDeclarationADefinition();
    }
...
void CalculateIwyuForForwardDeclareUse(
  ...
  const NamedDecl* tag_decl = DynCastFrom(use->decl());
  const ClassTemplateDecl* tpl_decl = DynCastFrom(use->decl());
  const ClassTemplateSpecializationDecl* spec_decl = DynCastFrom(use->decl());
  if (spec_decl)
    tpl_decl = spec_decl->getSpecializedTemplate();
...
void CleanupPrefixHeaderIncludes(
  ...
  for (OneIncludeOrForwardDeclareLine& line : *lines) {
    ...
    if (line.IsIncludeLine()) {
      ...
      const NamedDecl* dfn = GetTagDefinition(line.fwd_decl());
